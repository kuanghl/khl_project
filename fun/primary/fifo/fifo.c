...
int enque(queue * que, void * data)
{
    ...
    if (que->first == NULL) {
        ...
        que->last->next = new_node; //inter list
        que->last = new_node;
    }
    que->length++;
    print("Alloc new_node@ %p, inter queue length:%d\n", new_node, que->length);
    pthread_mutex_unlock(&mutex);
...
void * deque(queue * que) 
{
    ...
    if (que->first == que->last) {
        ...
        que->first = _node->next; // 下移
    }
    que->length--;
    data = _node->data;

    print("Freeing _node@ %p, out_queue length:%d\n", _node, que->length);
    free(_node);
    pthread_mutex_unlock(&mutex);
    // print("Exiting deque\n");
    return data;
